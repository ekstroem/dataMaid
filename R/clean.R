...
#' @param vol Extra text string that is appended on the end of the output file name(s), if file=NULL. 
#' For example, if the data set is called "myData", no file argument is supplied and vol="2", 
#' the output file will be called "cleanR_myData2.Rmd"
...
clean <- function(o, file=NULL, removeExisting=TRUE, maxnum=NULL,
  ...
  ## check function input and initial settings
    ...
    if (is.null(file) || substr(file, nchar(file)-3, nchar(file)) != ".Rmd") {
        ...
        #at least, tell the user if we're dropping their file name... 
    ...
    if (finish!="render") outFileExists <- FALSE
