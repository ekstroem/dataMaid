...
\citep{validate}, \pkg{assertr}, and \citep{assertr}. These packages
...
same authors) and \pkg{assertive} offers tools for identifying errors
...
\code{events} from \code{toyData}, \code{makeDataReport()} has identified two values that
...
\begin{figure}[tb]
\begin{center}
\includegraphics[width=7.5cm]{toyData-events.pdf}
\end{center}
\caption{Output from running \code{visualize()} on the variable \code{events} from the
