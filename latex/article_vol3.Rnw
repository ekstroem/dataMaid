...
<<echo=FALSE>>=
# options(prompt = "R> ", width=77, continue = "+  ")
...
frames from Microsoft Excel data files. \hl{Mention assertive here}. \pkg{dplyr}, \pkg{tidyr}, \pkg{data.table}
...
results were used in the data cleaning process. The \pkg{assertr} package provide very similar - and very nice - tools to those of \pkg{validate}, but without any amibitions of conducting auto-cleaning. 
...
by the second paradigm above. Next, we show atep-by-step how the data report and the interactive mode of \pkg{dataMaid} can be combined to conduct a well-documented, systematic data cleaning in Section \ref{sec:bigExample}. Here, we assess and clean a dirty dataset with information about the US presidents.  At last, in Section \ref{sec:specificExamples}, we discuss a number of examples of
...
shown in Figure~\ref{fig:example1} and the following two pages are shown in Figure~\ref{fig:example2}. \code{toyData} is a very
...
list of flagged issues, if any. For instance, as shown in Figure~\ref{fig:example2}, for the \code{numeric}-type variable
...
<<>>=
makeDataReport(toyData, preChecks = c("isKey", "isSupported"), replace=TRUE)
@
...
<<warning=FALSE>>=
complexData <- data.frame(complexVar = complex(100, real = 1:100, imaginary = 3), numericVar = 1:100)
makeDataReport(complexData, treatXasY=list(complex = "numeric"), replace=TRUE)
@ 
...
<<>>=
check(toyData$var2, checks = setChecks(numeric = "identifyMissing"))
@ 
...
The syntax in this code chunk can be read as follows: "Summarize \code{var2} in \code{toyData}, and for \code{numeric} variables, set the summaries to be the default summary functions, except \code{variableType} and \code{countMissing}." 
...
Moving forward, we might be interested in inspecting the contents of the "."-coded entry of \code{firstName} closer, as we \textit{do} know the first names of all the US presidents. We look at the last name for this president and fill in the first name correctly:
...
Finally, we create a new data report, adding the suffix "cleaned" to the title, as well as the file name, so that we have documentation of the current state of the dataset. We also decide that it might be sufficient to be able to inspect only the real part of the presidential favorite numbers and therefore, we choose to treat the \code{complex} variable \code{favoriteNumber} as a \code{numeric} variable:
...
\subsection[Incorporating dataMaid in a programmer's workflow]{Incorporating \pkg{dataMaid} in a programmer's workflow}
...
If the dataset is large and the time available for reading through the data report is scarce, it can be convenient to only make a report concerning the variabels that were flagged to be problematic. This can be achieved by using the \code{onlyProblematic} argument in
...
<<eval=FALSE>>=
makeDataReport(toyData, standAlone = FALSE)
@ 
...
Alternatively, one can create the usual output report, not render it and then manually edit the produced \proglang{R} markdown file as wished. This can be done with the following command:

<<eval=FALSE>>=
makeDataReport(toyData, render = FALSE, openResult = FALSE)
@ 
