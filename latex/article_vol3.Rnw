...
<<label=toydataevents,fig=TRUE,include=FALSE>>=
...
If the dataset is large and the time available for reading through the data report is scarce, it can be convenient to only make a report concerning the variabels that were flagged to be problematic. This can be achieved by using the \code{onlyProblematic} argument for
