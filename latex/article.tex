...
cleaning. \hl{Hov! Er det ikke et job for autocleanR?}
...
\hline
\R{output} & Type of output file to be produced (html, pdf or no file) & \R{"pdf"} \\
\R{ordering} & Ordering of the variables in the data summary & \R{"asIs"} \\
\R{onlyProblematic} & Should only variables flagged as problematic be included in the output? & \R{FALSE} \\
\R{maxProbVals} & Maximum number of problematic values to print, if any are found in data checks & \R{Inf} \\
\R{maxDecimals} & Maximum number of decimals to print for numeric problematic values & \R{2}
\end{tabular}
\caption{A selection of commonly used arguments of \R{clean()}.}
\label{table.cleanFormals}
\end{table}

In \pkg{cleanR} the \code{clean} function is the primary workhorse and knowledge of nothing more than this function is required to produce data cleaning outputs in \pkg{cleanR}. The data cleaning output itself is an overview document, intended for reading by humans, in either pdf or html format. 
...
\section{Behind the scenes} \label{sec:internals}
