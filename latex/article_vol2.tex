...
Though the discussion in the above paints a picture of cleanR as a user-friendly package which requires practically no knowledge of R, one should not be mistaken to think that it is not customizable. In fact, the main function of cleanR, \code{clean}, is mainly a tool for formatting the results from various checking-, summary- and visualization functions \hl{as described... blabla}. Thus, the actual work underlying a cleanR output file can be anything or nothing - depending on the arguments given to \code{clean}. \hl{something about interactive mode. }

This section consists of three parts. First, we describe how the \R{clean} function can be extended by adding custom prechecks, summaries, checks and visualizations. In order to do this, one needs to produce small functions that obey to the syntax of \R{cleanR}, and therefore, the minimal requirements of this syntax are introduced. However, if one wishes to also use these custom functions in the interactive \R{cleanR} mode and e.g. have them appear in a \R{allSummaryFunctions()} call, have nice printing functions etc., one needs a few extra steps when producing the functions. These steps are presented in the second part of this section. Lastly, we turn to an example of how either type of custom cleaning functions can be produced, used in \R{clean()} and used interactively.

Throughout this section, Table~\ref{table.functionTypes} serves as an overview of the requirements and contents of each of the three function types used in \R{cleanR}, namely \R{summaryFunction}s (used for creating summaries), \R{visualFunction}s (used for creating plots) and \R{checkFunction}s (used for performing variable checks). 


\subsection{Writing cleanR extensions: The minimal approach}


\subsection{Getting the full cleanR machinery on your side}
...
Now, let's say we wish to perform quality control on a slightly more complicated dataset than \R{toyData}. \hl{something about which dataset}. We will build four new functions and show both how they can be used interactively and how they can be integrated with the \R{clean()} function. These four new functions are:
\begin{description}
...
\item[\R{identifyColon}] A new \R{checkFunction} that flags variables that colons that appear before and after alphanumerical characters, e.g. practical for identifying autogenerated interaction effects. This function will be used in the \textit{check} step of \R{clean()}. 
\end{description}
First, let's define the \R{isID} function. We do not particularly wish to use this function interactively, so we will stick to the minimal requirements of a \R{checkFunction} used in \R{check()}. The function might then be defined by
\begin{Schunk}
\begin{Sinput}
...
\end{Sinput}
\end{Schunk} 
This is essentially all we need to do in order to include this function as precheck-function in \R{clean()}, so we will leave this function as is and move on to the next function, namely \R{mosaicVisual}. We will define this function such that it gets the full \R{cleanR} functionality. This can be done using the code 
\begin{Schunk}
\begin{Sinput}
...
Now, we are done with the definition of \R{mosaicVisual} and we can turn to the next function in line, \R{countZeros}. This \R{summaryFunction} in spe is defined in the following lines of code:
\begin{Schunk}
\begin{Sinput}
...
\hl{Data cleaning with extension here}
