...
\Abstract{Data cleaning and -validation are the first steps in any
  ...
  data, but not necessarily knowledge of \R{R}. Thereby, \pkg{dataMaid} 
...
The \R{clean()} function is the primary workhorse of \pkg{dataMaid} and
...
\R{allCheckFunctions()} functions in \pkg{dataMaid} to print overview
...
\section{Using dataMaid interactively}
...
\pkg{dataMaid} output document. Writing a \R{visualFunction} is slightly
...
Available memory may pose a problem with larger datasets ---
especially on machiens running Windows. The problem is, that older,
32-bit versions of Windows cannot necessary access all physical RAM,
while newer 64-bit versions of Windows cannot access virtual memory,
which makes \proglang{R} unnecessary slow.  Another solution, which is
especially relevant to Windows users due to %the unfortunate
the memory control for this operating system,
%combination of memory control in this operating system and
%\proglang{R},
%\hl{And also just R, right? Du har udkommenterert denne kommentar,
...
\pkg{rmarkdown} files that supersedes the upper size limit of, say,
...
\subsection{Using dataMaid for problem flagging}
...
\subsection{Including dataMaid documents in other files}
...
preamble YAML might look like and how the \R{dataMaid\_toyData.Rmd} file can
then be included:
{\small
\begin{Verbatim}
---
...
In the this example, the \R{dataMaid\_toyData.Rmd} file could have been created as follows:
\begin{Schunk}
\begin{Sinput}
