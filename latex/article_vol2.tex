...
\Abstract{Data cleaning and -validation are the first steps in any
  ...
  \R{R}. Thereby, \pkg{cleanR} aids the dialogue between data
  analysists and field experts, while also providing easy
  ...
  assessment and -cleaning. \hl{Hvad henvises der til med denne
    sætning?}
...
provide an unsupervised decision on what should be done to potential
errors in the dataset, it can be an extremely useful tool in tracking
down and flagging potential erroneous data and in providing
information for humans to easier identify errors based on the context.
...
and less on the general researcher with a knowledge about the specific
field and context of the available data. The \pkg{cleanR} package
...
\proglang{R} on which data cleaning decisions can be build. This quality
...
contributing to a inter-field dialogue about the data at
...
\ref{sec:usingcleanR}, we introduce the representative of the first
...
\hl{3}, we present the interactive mode of \pkg{cleanR}, as motivated
...
\hl{4}, we turn to the question of how such \pkg{cleanR} extensions
...
in Section 5 \hl{proper ref}, we discuss a number of examples of
...
\section{Creating a data cleaning overview} \label{sec:example1}\label{sec:usingcleanR}

The \R{clean()} function is the primary workhorse of \pkg{cleanR} and
this is the only function needed by the user if the standard battery
of tests are used to output the data cleaning summaries. The data
cleaning output itself is an overview document, intended for reading
...
small ($15$ rows and $6$ variables), artificial dataset, whose only purpose is to
...
By default, an \proglang{R} markdown file and rendered pdf overview
...
variable, a figure visualizing its distribution and potentially also a
list of flagged issues. For instance, in the \R{numeric}-type variable
...
The most commonly used arguments are summarized in
...
shown in Figure~\ref{figure:cleanStructure}. In the following we give
examples on how to use these parameters to influence the output of
\R{clean() }.
...
\smallskip Control input variable and summary\\
...
\quad \R{replace} & Overwrite an existing markdown file with the same name? & \R{FALSE} \\
...
\caption{Overall illustration of the stages undertaken when running
...
For example, to get a summary document in html format that only
contains the variables with potential problems, and with a limit of
...
The final rendering of the generated markdown file is controlled with
the \R{render} and \R{openResult} arguments which both default to
\R{TRUE} \hl{Bliver det ved med at være sandt?}. \R{render} determines
if the \proglang{R} markdown file produced should be rendered using
the \pkg{rmarkdown} package and \R{openResult} decides whether the
output html or pdf file should be opened. The following command
produces an \proglang{R} markdown file containing the data cleaning
process information but the markdown file is not rendered into html
...
\pkg{cleanR} uses three
different types of functions for performing all steps in the above,
namely \code{summaryFunction}s, \code{visualFunction}s and
\code{checkFunction}s.
By default, \R{clean()} runs all the summary, visualization, and check
functions currently implemented for each variable type as part of the
algorithm (see
Figure~\ref{figure:cleanStructure}). Table~\ref{table.SVCfunctions}
lists all these functions but we can also use the
\R{allSummaryFunctions()}, \R{allVisualFunctions()}, and
\R{allCheckFunctions()} function in \proglang{R} to print an overview
list. For example the implemented summary functions are:
...
Thus we can see, for example, that the summary table for a
\R{numeric}, \R{integer}, or \R{Date} variable will include
information about the minimum and maximum values, while all variable
classes will produce some kind of information about the central
tendency (i.e., mode or median).

We can control the default summaries and checks for each variable type
...
vector of summary or check functions that should be run for a
particular variable type and the default values for each of these
arguments (i.e., the functions from Table~\ref{table.SVCfunctions}
that are applied) can be obtained through the \R{defaultXXXChecks()}
...
\begin{Schunk}
\begin{Sinput}
> clean(toyData, output="html",  
        characterSummaries = c("variableType", "centralValue"),
...
\begin{Sinput}
> clean(toyData, output="html",  
        allSummaries = c("variableType", "centralValue"))
\end{Sinput}
\end{Schunk}
...
\begin{Sinput}
> clean(toyData, output="html",  
        factorChecks = c("identifyWhitespace"))
\end{Sinput}
\end{Schunk}

or we could remove checks for factor altogether by setting the
corresponding argument to \R{NULL} in which case factor variables will
not be checked for any potential errors.

\begin{Schunk}
\begin{Sinput}
> clean(toyData, output="html", factorChecks = NULL)
\end{Sinput}
\end{Schunk}
...
where \R{clean()} applies function(s) to each of the variables: 
\begin{enumerate}
...
instance the dataset at hand requires completely different
visualizations then more control is needed, and
Section~\ref{sec:extending} explains how to modify, and expand the
possibilities by producing new functions quite easily.

\section{Using cleanR interactively}
While overview documents are great for presenting and documenting the
data cleaning checks, it may be natural to work more interactively
through the data cleaning process. \pkg{cleanR} also
...
Table~\ref{table.SVCfunctions} or the \R{allCheckFunctions()} provides
...
such as \R{identifyMissing} (see \ref{table.SVCfunctions} for a list
of check functions) is by using it directly on the variable, e.g.,
\begin{Schunk}
\begin{Sinput}
...
providing a vector of functions to run for a particular summary. To
only get the minimum and maximum we set
\R{numericSummaries=c("minMax")}.

\begin{Schunk}
\begin{Sinput}
> summarize(toyData$var2, numericSummaries=c("minMax"))
\end{Sinput}
\begin{Soutput}
     Feature         Result  
[1,] "Min. and max." "1; 999"
...
shown in the previous section. However, \pkg{cleanR} is fully
...
(e.g., \R{factorSummaries}, or \R{numericChecks}). However, the
functions used in the summarize/visualize/check steps need to adhere
to a specific structure to be called from these three steps, and
therefore, we will now present how \R{summaryFunction}s,
\R{visualFunction}s and \R{checkFunction}s are made.
%\hl{Mention something about the interactive mode here as well?} 

This section consists of two parts. First, we describe how the
\R{clean()} function can be extended by adding custom prechecks,
summaries, checks and visualizations. In order to do this, one needs
to create functions with a specific structure required by
\pkg{cleanR}. This can be done with different levels of strictness. If
the custom functions are only to be used part of the corresponding
arguments of \R{clean()} then only the input/output structures of the
...
be recognized and behave in the same way as the build-in functionson
...
function types. We end this section with a 
%After this abstract, templatic overview of the internals of the three
%SVC functions is given, we turn to a 
worked example of how to use custom made functions in practice, where
four new summarize/visualize/check functions are defined and used,
both interactively and in \R{clean()}.
...
object. This is done by calling the \R{summaryFunction} function with
the user-defined function as first argument, and additional arguments
\R{description} which adds explanatory flavour text as an attribute of
the function, and \R{classes} which determines which classes the
user-defined function should be applied to (also stored as an
attribute).
\begin{Verbatim}
...
stored as an attribute. What happens if the \R{classes} argument is
not specified depends on the type of \R{mySummaryFunction}. If
\R{mySummaryFunction} is a \R{S3} generic function with associated
...
empty. Note that the helper function \R{allClasses()} might be useful
...
use. See Table~\ref{table.functionTypes}.


Similarly to the summary function, we call \R{visualFunction()} to
register our newly created function. Like before, it accepts the
...
Again, registering the visualization function allows it to be used
without manually adding it to the list of visualizations.
...
\end{Verbatim}
For this template, the argument \R{maxDecimals} is not in use. This
argument is used to round off the \R{problemValues} passed to
\R{messageGenerator()}, if they are numerical.  This is done by
substituting the \R{problemStatus} assignment above with the following
code: \hl{Denne foregående sætning forstår jeg ikke helt. Måske skal
  messagegenerator have en sætning eller to med på vejen? (Og det spiller lidt sammen med nedenstående)}
\begin{Schunk}
\begin{Sinput}
problemStatus <- list(problem = problem, 
...
%Another noteworthy component of the template is the usage of the
helper function \R{messageGenerator()}, which aids in consistent
styling of all \code{checkFunction} messages. This function simply
pastes together the \code{problemMessage} and the
\code{problemValues}, with the latter being quoted and sorted
alphabetically. If \R{nMax} is not \R{Inf}, only the first \R{nMax}
problem values will be pasted onto the message, accompanied by a
comment about how many problem values were left out (if any).  Note
that printing quotes in \pkg{rmarkdown} requires an extensive amount
of character escaping, so opting for \code{messageGenerator()} really
...
supposed to list problematic values in the variable, it falls within
...
\end{Sinput}
\end{Schunk}
This function can now be called directly or used in \R{clean()} and we
will show an example below. Depending on the \R{doEval} argument
\hl{skal doEval have en default value ovenfor?}, either a text string
...
appear in a \R{allVisualFunctions()} call. In order to get this
...
> clean(testData, 
      ...
      # Add the new checks for specific data types
...
\section{Using cleanR in specific situations}
Finally, we will present a few examples of how to make \pkg{cleanR}
...
slow, while a large number of observations generally will affect the
...
and RStudio %\hl{And also just R, right? There's got to be a nice
%  reference on this..?}, 
...
typically be for very large datasets, having this file opened in memory
...
\pkg{rmarkdown} files that supersedes the upper size limit.
% of RStudio,
...
Sometimes, a \pkg{cleanR} document might be a useful addition to a more general overview document, including also e.g. pairwise association plots, time series plots or exploratory analysis results. To this end, it is possible to produce a \pkg{cleanR} document that can readily be included in other \R{rmarkdown} files. This is done by using the \R{standAlone} argument in \R{clean}, which removes the preamble from the outputted \R{rmarkdown} file. Please note, that it is still necessary to indicate which \R{rmarkdown} type is being created; the pdf and html \R{rmarkdown} styles are unfortunately not identical. 

If it is important that the embedded \pkg{cleanR} document can be rendered to either of these two file types, we recommend setting \R{twoCols = FALSE} and \R{output = html} in \R{clean()}, thereby essentially removing almost all output type specific formatting code from the generated \R{rmarkdown} file. 
...
\section{Appendix Something} \label{sec:appendix1}
\includepdf[pages=2-4, pagecommand={}, frame=true, noautoscale=true, scale=0.7]{cleanR_toyData.pdf}

\section{Appendix NUMTWO} \label{sec:appendix2}
