...
\Abstract{Data cleaning and -validation are the first steps in any
  ...
  data, but not necessarily knowledge of \proglang{R}. Thereby, \pkg{dataMaid}
...
\proglang{R}-packages such as \pkg{plyr} \citep{plyr}, and \pkg{data.table} have
...
field and the context of the available data. The \pkg{dataMaid} package
...
We begin with an example that is intended as an illustration of how \code{clean()} might
...
(output not shown). Also, we can add the argument \code{replace=TRUE} in
order to force \code{clean()} to overwrite any existing files produced by
\code{clean()}, as we might run this function a few times in a row before deciding
exactly which cleaning overview we want. Using the \code{toyData} dataset as a guinea pig, we type:
...
& Description &  \multicolumn{7}{c}{Variable classes} \\ \smallskip
 ...
 \textbf{\code{visualFunction}s} \smallskip \\
  ...
  \quad \code{standardVisual} & Histograms and barplots using ggplot2 &  $\times$ & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
  \smallskip \\
...
Thus we can see, for example, that for \code{numeric}, \code{integer}, or \code{Date}
variables, \pkg{dataMaid} provides functions for adding summary information about
the minimum and maximum values, while all seven variable
classes dealt with in \pkg{dataMaid} have functions for central
...
factor variables, then we would need to set the \code{factorChecks} accordingly
...
arguments in \code{clean()}, and above we have shown examples of how to
...
through the data cleaning process as well. Aside from the \code{clean()} function
presented above, \pkg{dataMaid} also provides more standard \proglang{R} interactive tools, such as
...
functions \code{check()}, \code{summarize()} and \code{visualize()} to work
interactively with \pkg{dataMaid}.
...
\code{problemValues}. If we for instance decide that the values \code{999}
...
\end{center}
\label{fig:example2}
\caption{Output from running \code{visualize()} on the variable \texttt{var2} from the
...
This section consists of two parts. First, we describe how customized \code{summaryFunction}s,
\code{visualFunction}s and \code{checkFunctions} can be made, including an introduction to a
few convenient tools available in \pkg{dataMaid} for aiding SVC-function construction.
...
four new SVC functions are defined and used, both interactively and in \code{clean()}.
...
Output (required) & A list with entries: \newline \code{\$feature} - a label for the summary value (as character string); \newline \code{\$result} - the result of the summary (as character string) & A character string with \code{R} code for producing a plot. This code should be standalone, i.e., should include the data if necessary. & A list with entries: \newline \code{\$problem} - a logical identifying whether an issue was found; \newline \code{\$message} - a character string (possibly empty) decribing the issue that was found, properly escaped and ready for use in \code{rmarkdown} \\
...
\hspace*{-.2cm}\begin{tabular}[t]{p{0.45\linewidth} |
                 ...
                 opened by the graphic device in \code{R}. &
                                                          \vspace*{-1cm}\raggedright If \code{doEval} is \code{FALSE}: \newline   A text string with \code{R} code, as described above. \end{tabular} \egroup  & A \code{checkResult} object, i.e., an attributed list with entries \code{\$problem}, \code{\$message} and \code{\$problemValues}, the latter being either \code{NULL} or the problem causing values, as they were found in \code{v}, whichever is relevant. \\
...
\end{Verbatim}
The last function called here, \code{summaryResult()}, changes the class
of the output, thereby making a \code{print()} method available for it.
...
If the \code{description} argument is left unspecified, the name of the
function (which in this case is \code{"mySummaryFunction"}) will be filled in and
stored under the \code{description} attribute. What happens if the \code{classes} argument is
not specified depends on the type of \code{mySummaryFunction}. If
\code{mySummaryFunction} is constructed as an \code{S3} generic function with associated
methods, the call to \code{summaryFunction()} will automatically produce
...
\code{classes} is left unspecified, the attribute will simply be
left empty. Note that the helper function \code{allClasses()} might be useful
for filling out the \code{classes} argument, as it simply lists all
available classes in \pkg{dataMaid}:
...
the fact that \code{visualFunctions} need to be able to output standalone
...
in the outputted \code{checkResult}.
...
\begin{description}
\item[\code{isID}] A new \code{checkFunction} intended for use in the precheck-stage. This function checks whether a variable consists exclusively of long ($> 10$ characters/digits) entries that are all of equal length, as this might be personal identification codes that we do not wish to print out in the data summary.
...
First, let's define the \code{isID()} function. As this function is not
supposed to list problematic values, it falls within
the category of \code{checkFunctions} represented by
\code{myFullVarCheckFunction()} in the above. We do not particularly wish
...
function as a precheck-function in \code{clean()}, so we will leave it as
is and move on to the next function, namely \code{mosaicVisual()}.

\subsubsection{mosiacVisual --- a new visualFunction}
\proglang{R} contains a function, \code{mosiacplot()}, which produces
...
there is no difference between the entries \code{\$result} and
...
The last function mentioned above is \code{identifyColons()}. We define
it using the helper function \code{messageGenerator()} to obtain a properly
escaped message, and we use \code{checkResult} to make its output print
neatly:
\begin{Schunk}
\begin{Sinput}
...
however, that for \code{checkFunction}s, the function description will
appear in the document produced by \code{clean()} (in the \textit{Data
  cleaning summary} section of the output), so now this is not only
done for the sake of the \code{allCheckFunctions()} output.
...
\item We want the new check function, \code{identifyColon}, to be added to the checks performed on \code{character}, \code{factor} and \code{labelled} variables.
\end{itemize}
...
\pkg{dataMaid} output document can be included in other \pkg{rmarkdown}
...
documents. Note that the interactive tools of \pkg{dataMaid} can be used
...
by \code{clean()}, namely producing the \pkg{rmarkdown} file and rendering it
...
\pkg{rmarkdown} files that supersedes the upper size limit of code editors, e.g.,
...
be useful. This can be achieved by using the \code{onlyProblematic}
argument in \code{clean()}. By specifying \code{onlyProblematic = TRUE},
...
done by also controlling the \code{mode} argument:
\begin{Schunk}
\begin{Sinput}
> clean(toyData, onlyProblematic = TRUE, mode = c("check"))
\end{Sinput}
\end{Schunk}
Now only the checking results are printed, and only for variables where problems were identified. An even more minimal output can be generated by also leaving out the checking results --- then \code{clean()} essentially just produces a list of the variable names that should be investigated further:
\begin{Schunk}
\begin{Sinput}
...
results. To this end, it is possible to produce a \pkg{dataMaid}
document that can readily be included in other \pkg{rmarkdown}
files. This is done by using the \code{standAlone} argument in \code{clean()},
which removes the preamble from the outputted \pkg{rmarkdown}
file. Please note, that it is still necessary to indicate which
\code{rmarkdown} type is being created; the pdf and html \pkg{rmarkdown}
...
In the this example, the \code{dataMaid\_toyData.Rmd} file could have been created as follows:
\begin{Schunk}
\begin{Sinput}
...
\end{Sinput}
\end{Schunk}
Note that with the latter option, no special YAML preamble is needed in the \pkg{rmarkdown} document.
