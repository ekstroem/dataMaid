...
\pkg{data.table} \citep{data.table} have
made data wrangling a lot easier, but only a handful of packages such
as \pkg{editrules} \citep{editrules}, \pkg{validate} \citep{validate},
\pkg{DataCombine} \citep{DataCombine}, and
\pkg{janitor} \citep{janitor} attempt to implement systematic, reproducible data
