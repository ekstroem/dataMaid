...
makeDataReport(exampleData, 
      ...
      output = "html",
      open = FALSE,
      reportTitle = "exampleData Report from beginning of vignette",
      vol = "1")
``` 

#Introduction
`dataMaid` can be used as a user-friendly, self-contained package, as outlined in our [article manuscript](https://github.com/ekstroem/dataMaid/blob/master/latex/article_vol3.pdf). However, `dataMaid` was built specifically with the purpose of 

reports are fully customizable and this vignette was made as a tutorial to how one can write simple extensions that can be used in e.g. the `makeDataReport()` function of `dataMaid`. 

In order to understand



and `makeDataReport()` is mainly a tool for formatting the
results from various checking, summary,  and visualization
functions. Thus, the actual work underlying a `dataMaid` output file
can be anything --- depending on the arguments given to `makeDataReport()`
...
This vignette consists of two parts. First, we describe how customized
...
construction.  The table below serves as an overview
...
interactively and in `makeDataReport()`.
...
1. **`isID()`**: A new `checkFunction` intended for use in the pre-check-stage. This function checks whether a variable consists exclusively of long ($> 10$ characters/digits) entries that are all of equal length, as this might be personal identification codes that we do not wish to print out in the data summary.
2. **`mosaicVisual()`**: A new `visualFunction` that produces mosaic plots. This function will be used in the *visualize* step of `makeDataReport()`.
3. **`countZeros()`**: A new `summaryFunction` that counts the number of occurrences of the value `0` in a variable. This function will be used in the *summarize* step of `makeDataReport()`.
4. **`identifyColons()`** A new `checkFunction` that flags variables in which values have colons that appear before and after alphanumerical characters. This is practical for identifying autogenerated interaction effects. This function will be used in the *check* step of `makeDataReport()`.
...
```{r makeSecondReport}
data("toyData")
makeDataReport(toyData, replace = TRUE, output = "html")
``` 
...
makeDataReport(exampleData,
      ...
      output = "html",
      open = FALSE,
      reportTitle = "exampleData report from end of vignette",
      vol = "2")
```  
...
```{r hithere}
htmltools::includeHTML("dataMaid_toyData.html")
```
