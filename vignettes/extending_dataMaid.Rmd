...
`dataMaid` can be used as a user-friendly, self-contained package, as shown in the previous sections. However, `dataMaid` is fully customizable and `makeDataReport()` is mainly a tool for formatting the
...
We have chosen the output to be html so that it can easily be included in this vignette by use of the .

The outputted report is available at the end of this vignette. A few comments 
...
```{r, eval  =FALSE}
library(htmltools)
system("cp dataMaid_exampleData.html ~/xxx.html")
includeHTML("dataMaid_exampleData.html")
```

```{r, child = "dataMaid_exampleData.Rmd"}
```
