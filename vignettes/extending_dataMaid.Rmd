...
`dataMaid` can be used as a user-friendly, self-contained package, as shown in the previous sections. However, `dataMaid` is fully customizable and `clean()` is mainly a tool for formatting the
results from various checking, summary,  and visualization
functions. Thus, the actual work underlying a `dataMaid` output file
can be anything --- depending on the arguments given to `clean()`
...
therefore, we will now present how `summaryFunction`s,
`visualFunction`s and `checkFunction`s are made.


This section consists of two parts. First, we describe how customized
`summaryFunction`s, `visualFunction`s and
`checkFunctions` can be made, including an introduction to a few
convenient tools available in `dataMaid` for aiding SVC-function
...
\hline
\vspace{0pt} Input (required) & `v` --- a variable vector.
                                \newline `...` --- additional
                                arguments passed to the function.  &  `v` --- a variable vector. \newline `vnam` --- the variable name (as character string). \newline `doEval` --- a logical (`TRUE`/`FALSE`) controlling the output type of the function. & `v` - a variable vector \newline `nMax` - an integer (or `Inf`), controlling how many problematic values are printed, if relevant \newline `...` --- additional arguments passed to the function. \\
...
Output (required) & A list with entries: \newline `\$feature` --- a label for the summary value (as character string); \newline `\$result` --- the result of the summary (as character string). & A character string with `R` code for producing a plot. This code should be standalone, i.e., should include the data if necessary. & A list with entries: \newline `\$problem` --- a logical identifying whether an issue was found; \newline `\$message` --- a character string (possibly empty) decribing the issue that was found, properly escaped and ready for use with `rmarkdown`. \\
Output (recommended) & A `summaryResult` object, i.e., an attributed list with entries `\$feature`, `\$result` and `\$value`, the latter being the values from `\$result` in their original format). & \bgroup
\def\arraystretch{1.8}%
\hspace*{-.2cm}\begin{tabular}[t]{p{0.45\linewidth} |
                 p{0.45\linewidth}} \vspace*{-1cm}\raggedright If
                 `doEval` is `TRUE`: \newline A plot that is
                 opened by the graphic device in `R`. &
                                                          \vspace*{-1cm}\raggedright If `doEval` is `FALSE`: \newline   A text string with `R` code, as described above. \end{tabular} \egroup  & A `checkResult` object, i.e., an attributed list with entries `\$problem`, `\$message` and `\$problemValues`, the latter being either `NULL` or the problem causing values, as they were found in `v`, whichever is relevant. \\
Tools available for producing the function & `summaryResult()` & --- & `messageGenerator()` \newline `checkResult()`
\end{tabular}
\egroup
...
the new custom function is only to be used as part of the SVC steps in `clean()`,
...
made `mySummaryFunction()` in `clean()`, but we recommend
...
`allSummaryFunctions()` call.
% One comment should be devoted to
% the two attributes of a `summaryFunction`.
If the `description` argument is left unspecified, the name of
the function (which in this case is `"mySummaryFunction"`) will
be filled in and used as the `description` attribute. What
happens if the `classes` argument is not specified depends on the
type of `summaryFunction`. If `mySummaryFunction` is
...
code for plots in order for `clean()` to build standalone
...
strictly necessary for a `visualFunction`'s use in `clean()`, but
...
used in the summarize/visualize/check steps in `clean()`, the
...
used interactively and how they can be integrated with the `clean()`
function. These four new functions are:
\begin{description}
\item[`isID()`] A new `checkFunction` intended for use in the pre-check-stage. This function checks whether a variable consists exclusively of long ($> 10$ characters/digits) entries that are all of equal length, as this might be personal identification codes that we do not wish to print out in the data summary.
\item[`mosaicVisual()`] A new `visualFunction` that produces mosaic plots. This function will be used in the *visualize* step of `clean()`.
\item[`countZeros()`] A new `summaryFunction` that counts the number of occurrences of the value `0` in a variable. This function will be used in the *summarize* step of `clean()`.
\item[`identifyColons()`] A new `checkFunction` that flags variables in which values have colons that appear before and after alphanumerical characters. This is practical for identifying autogenerated interaction effects. This function will be used in the *check* step of `clean()`.
\end{description}
...
function as a pre-check-function in `clean()`, so we will leave it as
...
This function can now be called directly or used in `clean()`, as will
...
the document produced by `clean()` (in the *Data cleaning
...
Now, we are ready to use these new functions in a `clean()`
...
\begin{itemize}
\item We want to add the new pre-check function, `isID()`, to the already existing pre-checks.
\item We wish to change the plot type for all variables to the new mosaic plot.
\item We want the new summary function, `countZeros()`, to be added to the summaries performed on all variable types but `Date` and `logical`.
\item We want the new check function, `identifyColons()`, to be added to the checks performed on `character`, `factor` and `labelled` variables.
\end{itemize}
