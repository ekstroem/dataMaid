...
1. The result of a single eligiblity check (refered to as a *pre-check* below)
...
The first option will be used for variables that do not pass the initial pre-checks. These are checks that are performed in order to identify variables that are not eligable for detailed documentation for one reason or another. The default pre-checks used in `makeDataReport()` are 
...
meanSummaryHelper <- function(v, maxDecimals) {
  ...
  #store printable (rounded) putput in `res`
...
mosaicVisual <- function(v, vnam, doEval) {
  ...
  #if doEval is TRUE, evaluate the call, thereby prooducing a plot
...
#Make it a proper visualFunctin:
...
We specify that the `prettierHist()` should only be called on variables of the classes `numeric`, `integer`, `logical` or `Date`, as is reasonable for a histogram. And we will now find our new `visualFunctions` in a `allVisualFunction()`-call:
...
identifyColons <- function(v, nMax = Inf, ... ) {
  #remove dupliates (for speed) and missing values:
  v <- unique(na.omit(v))
  ...
  #Idenitfy values in v that has the structure: First something (.), 
